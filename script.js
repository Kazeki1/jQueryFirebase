$(document).ready(function () {
  ...
  $('#dynamicBtn').click(function () {
    ...
    if ($(this).text() == "Save Changes") {
      //add an employee with document name as (first letterof fname and lname)
      ...
      sfDocRef.set({
        ...
        .catch(function (error) {
          $('#operationStatus').html('<div class="alert alert-danger"><strong>Failure!</strong>Employee not updated</div>').delay(2500).fadeOut('slow');
        });
    }
  });
  ...
  $("tbody.tbodyData").on("click", "td.deleteEmployee", function () {
    ...
    var lName = $(this).closest('tr').find('.lname').text(); //Last Name
      ...
      db.collection("employees").doc(docuName).delete().then(function() {
        $('#operationStatus').html('<div class="alert alert-success"><strong>Success!></strong> Employee deleted</div>').delay(2500).fadeOut('slow');
        LoadData();
      }).catch(function(error) {
        $('#operationStatus').html('<div class="alert alert-danger"><strong>Failure!</strong> Employee not deleted</div>').delay(2500).fadeOut('slow');
      });
    });
